    ...
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",
      ...
      <div style={{ background:"#ffffff",borderRadius:12,padding:"28px",
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
          <h2 style={{ margin:0,fontSize:16,fontWeight:700,color:"#111",letterSpacing:"0.01em" }}>{title}</h2>
          <button onClick={onClose} style={{ background:"none",border:"none",cursor:"pointer",fontSize:24,color:"#999",lineHeight:1,padding:4 }}>×</button>
        </div>
        {children}
      </div>
    </div>
...
// ─── EMAIL PREVIEW ────────────────────────────────────────────────
function EmailPreview({ container, recipientEmail, onClose, onSend, isSending }) {
  ...
  const body = [
    ...
    ...(missing.length>0 ? [``, `Missing Information:`, ...missing.map(m=>`  • ${m}`)] : []),
    ...(risks.length>0 ? [``, `Risks Identified:`, ...risks.map(r=>`  ⚠ ${r}`)] : []),
    ``,
    `Please confirm or update by end of day.`,
    ``,
  ...
  return (
    <div>
      <p style={{ fontSize:13,color:"#555",marginTop:0,lineHeight:1.6 }}>
        ...
        <code style={{ fontSize:11,background:"#f0f0f0",padding:"2px 6px",borderRadius:3,color:"#333" }}>
          ContainerNo, Carrier, ETA (YYYY-MM-DD), Port, Destination, Account
        </code>
      </p>
      ...
      <div style={{ display:"flex",gap:10,justifyContent:"flex-end",marginTop:16,borderTop:"1px solid #eee",paddingTop:16 }}>
        <button onClick={onClose}
          style={{ padding:"8px 18px",borderRadius:6,border:"1px solid #ddd",background:"#fff",cursor:"pointer",fontSize:13,color:"#555",fontFamily:"inherit" }}>Cancel</button>
        <button onClick={handle}
          style={{ padding:"8px 18px",borderRadius:6,border:"none",background:"#111",color:"#fff",fontWeight:700,cursor:"pointer",fontSize:13,fontFamily:"inherit" }}>Import</button>
      </div>
    </div>
  );
}
...
function SettingsModal({ settings, onSave, onClose }) {
  ...
  return (
    <div>
      <div style={{ marginBottom:14 }}>
        <FieldLabel>Alert Recipient Email(s) — comma separated</FieldLabel>
        <input style={lightInp} value={form.alertEmail||""}
          onChange={e => setForm(f => ({ ...f, alertEmail:e.target.value }))}
          placeholder="ops@sourceone.global, manager@sourceone.global" />
      </div>
      <div style={{ marginBottom:20 }}>
        <FieldLabel>Team / Company Name</FieldLabel>
        <input style={lightInp} value={form.teamName||""}
          onChange={e => setForm(f => ({ ...f, teamName:e.target.value }))}
          placeholder="Source One Global" />
      </div>
      <div style={{ display:"flex",gap:10,justifyContent:"flex-end",borderTop:"1px solid #eee",paddingTop:16 }}>
        <button onClick={onClose}
          style={{ padding:"8px 18px",borderRadius:6,border:"1px solid #ddd",background:"#fff",cursor:"pointer",fontSize:13,color:"#555",fontFamily:"inherit" }}>Cancel</button>
        <button onClick={() => onSave(form)}
          style={{ padding:"8px 20px",borderRadius:6,border:"none",background:"#111",color:"#fff",fontWeight:700,cursor:"pointer",fontSize:13,fontFamily:"inherit" }}>Save Settings</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const th = { padding:"10px 12px",fontSize:10,color:"#444",fontWeight:700,textAlign:"left",
    borderBottom:"1px solid #1e1e1e",whiteSpace:"nowrap",letterSpacing:"0.06em",textTransform:"uppercase" };
  ...
  return (
    <div style={{ fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",
      ...
      <style>{`
        ...
        body{background:#0a0a0a!important;color:#fff!important;margin:0}
        #root{background:#0a0a0a!important;min-height:100vh}
      ...
      <div style={{ background:"#111",borderBottom:"1px solid #1e1e1e",padding:"0 24px",
        ...
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          ...
          <div>
            <div style={{ fontSize:12,fontWeight:800,letterSpacing:"0.08em",color:"#fff",lineHeight:1.1,textTransform:"uppercase" }}>Source One Global</div>
            <div style={{ fontSize:9,color:"#444",letterSpacing:"0.14em",textTransform:"uppercase",lineHeight:1.2 }}>Container Control Tower</div>
          </div>
        </div>
        <div style={{ width:1,height:24,background:"#222",margin:"0 4px" }}/>
        <span style={{ fontSize:12,color:"#444" }}>{containers.length.toLocaleString()} containers</span>
        <div style={{ display:"flex",alignItems:"center",gap:5 }}>
          <span style={{ width:6,height:6,borderRadius:"50%",background:"#22c55e",display:"inline-block" }}/>
          <span style={{ fontSize:11,color:"#22c55e" }}>Live</span>
        </div>
        {lastRefresh && <span style={{ fontSize:11,color:"#333" }}>↻ {lastRefresh.toLocaleTimeString()}</span>}
        <div style={{ flex:1 }}/>
        {alertsDue.length>0 && (
          <span style={{ background:"rgba(248,113,113,0.12)",color:"#f87171",fontSize:11,
            padding:"4px 10px",borderRadius:4,fontWeight:600,border:"1px solid rgba(248,113,113,0.25)" }}>
            ⚠ {alertsDue.length} alert{alertsDue.length>1?"s":""} due
          </span>
        )}
        <button onClick={loadContainers}
          style={{ padding:"6px 10px",borderRadius:6,border:"1px solid #222",background:"transparent",cursor:"pointer",fontSize:13,color:"#444",fontFamily:"inherit" }}>↻</button>
        <button onClick={() => setShowImport(true)}
          style={{ padding:"6px 12px",borderRadius:6,border:"1px solid #222",background:"transparent",cursor:"pointer",fontSize:12,color:"#666",fontFamily:"inherit" }}>↑ Import CSV</button>
        ...
        <button onClick={() => setShowSettings(true)}
          style={{ padding:"6px 10px",borderRadius:6,border:"1px solid #222",background:"transparent",cursor:"pointer",fontSize:15,color:"#444",fontFamily:"inherit" }}>⚙</button>
      </div>
      ...
      <div style={{ padding:"20px 24px 0",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:12 }}>
        {stats.map(s => (
          <div key={s.label} style={{ background:"#111",border:"1px solid #1e1e1e",borderRadius:8,
            ...
            <div style={{ fontSize:26,fontWeight:800,color:s.value>0?s.color:"#2a2a2a",letterSpacing:"-0.02em" }}>{s.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding:"16px 24px",display:"flex",gap:10,flexWrap:"wrap",alignItems:"center" }}>
        <input style={{ ...darkInp,width:240 }} placeholder="Search container, port, destination…"
        ...
        <select style={{ ...darkInp,width:140 }} value={filterAccount} onChange={e=>setFilterAccount(e.target.value)}>
          {accounts.map(a => <option key={a} value={a} style={{ background:"#1a1a1a",color:"#fff" }}>{a}</option>)}
        </select>
        <span style={{ fontSize:11,color:"#333",marginLeft:"auto" }}>{filtered.length.toLocaleString()} / {containers.length.toLocaleString()}</span>
      </div>
      ...
      <div style={{ padding:"0 24px 60px",overflowX:"auto" }}>
        {loading ? (
          <div style={{ textAlign:"center",padding:"80px 0",color:"#333" }}>
            ...
            <p style={{ fontSize:13,marginTop:8 }}>Connecting to database…</p>
          </div>
        ) : containers.length===0 && !dbError ? (
          <div style={{ textAlign:"center",padding:"80px 0",color:"#444" }}>
            <p style={{ fontSize:14,marginBottom:4,color:"#555" }}>No containers tracked yet.</p>
              ...
              <thead>
                <tr style={{ background:"#161616" }}>
                  {[["containerNumber","Container"],["account","Account"],["eta","ETA"],["port","Port"],["destination","Destination"],["status","Status"]].map(([k,l]) => (
                    <th key={k} style={th}><button onClick={()=>handleSort(k)} style={{background:"none",border:"none",cursor:"pointer",fontSize:10,fontWeight:sortKey===k?700:400,color:sortKey===k?"#ffffff":"#555",padding:"0 2px",whiteSpace:"nowrap",letterSpacing:"0.06em",textTransform:"uppercase",fontFamily:"inherit"}}>{l}{sortKey===k?(sortDir==="asc"?" ↑":" ↓"):""}</button></th>
                  ))}
                ...
                {filtered.map(c => {
                  ...
                  return (
                    <tr key={c.containerNumber}>
                      ...
                      <td style={td}>
                        {c.account
                          ? <span style={{ fontSize:11,background:"#1a1a1a",color:"#777",padding:"2px 7px",borderRadius:3,border:"1px solid #2a2a2a",fontWeight:500 }}>{c.account}</span>
                          : <span style={{ color:"#2a2a2a" }}>—</span>}
                      </td>
                      <td style={td}>
                        <span style={{ fontSize:12,color:c.eta?"#ccc":"#f87171",fontFamily:"monospace" }}>{c.eta||"Missing"}</span>
                        {c.eta&&<div style={{ fontSize:10,color:"#333",marginTop:2 }}>↻ {getReminderDate(c.eta)?.toLocaleDateString()||"—"}</div>}
                      </td>
                      ...
                      <td style={td}>
                        <span style={{ display:"inline-flex",alignItems:"center",gap:5,fontSize:11,fontWeight:500,color:STATUS_DOT[c.status]||"#888",whiteSpace:"nowrap" }}>
                      ...
                      <td style={td}>
                        {(c.truckerWarehouse||c.truckerDistributor)
                          ? <span style={{ fontSize:11,color:"#777" }}>{c.truckerWarehouse||c.truckerDistributor}</span>
